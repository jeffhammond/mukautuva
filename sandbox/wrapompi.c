...
MUK_Comm MUK_COMM_WORLD = (void*)&IMPL_COMM_WORLD;
MUK_Comm MUK_COMM_SELF  = (void*)&IMPL_COMM_SELF;

// Open-MPI handles are incomplete pointers, but to have symmetry with the MPICH wrapper,
// we will make MUK handles a pointer to the OMPI handles and dereference them the same.
// muuntaa = convert
