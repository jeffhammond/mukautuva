int add_persistent_request_alltoallw_buffers(MPI_Request request, MPI_Datatype * sendtypes, MPI_Datatype * recvtypes)
{
    ...
    auto [it,rc] = request_persistent_ialltoallw_map.insert_or_assign(request, std::make_pair(sendtypes, recvtypes));
#if DEBUG
...
{
    try {
        auto [stypes,rtypes] = request_persistent_ialltoallw_map.at(request);
#if DEBUG
...
int remove_persistent_request_alltoallw_buffers(MPI_Request request)
{
    ...
    return request_persistent_ialltoallw_map.erase(request);
}
